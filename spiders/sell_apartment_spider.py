import scrapy
#Command
#scrapy crawl --set FEED_EXPORT_ENCODING=utf-8 sell_apartment -o sell_apartment.json

class AuthorSpider(scrapy.Spider):
    name = 'sell_apartment'

    start_urls = ['http://batdongsan.com.vn/ban-can-ho-chung-cu/p1']
    ...
    def parse_item(self, response):
        ...
        project_name = value2[label2.index('tên dự án:')] if (label2.count('tên dự án:') > 0) else ''
        investor = value2[label2.index('chủ đầu tư:') + 2] if (label2.count('chủ đầu tư:') > 0) else ''
        scale = value2[label2.index('quy mô:') + 2] if (label2.count('quy mô:') > 0) else ''
